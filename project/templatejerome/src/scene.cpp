...
int i,nbModel,nbShader;
  ...
  if(file){
    ...
    for(i=0;i<nbModel;++i)
    {
      getline(file, line); 
      stringstream iss(line);
      while(iss >> modelName >> pathModel >> modelShader )
      {
        this->models[modelName] = Model((char*)pathModel.c_str(),(char*)modelShader.c_str());
      }
    }   
